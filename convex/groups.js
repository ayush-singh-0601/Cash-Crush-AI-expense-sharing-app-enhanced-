  ...
  handler: async (ctx, { groupId }) => {
    ...
    const memberDetailsRaw = await Promise.all(
      group.members.map(async (m) => {
        ...
        if (!u) return null;
        return { id: u._id, name: u.name, email: u.email, imageUrl: u.imageUrl, role: m.role };
      })
    );
    ...
    for (const exp of expenses) {
      ...
      for (const split of exp.splits) {
        if (!ids.includes(split.userId)) continue;
    ...
    const balances = memberDetails
      .filter((m) => m && m.id in totals && m.id in ledger)
      .map((m) => ({
        ...m,
    ...
    memberDetails.forEach((member) => {
      if (member && member.id) userLookupMap[member.id] = member;
    });
...
export { editGroup, removeGroupMember, deleteGroup } from "./contacts";
